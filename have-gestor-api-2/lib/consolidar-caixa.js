...
async function consolidarAnual(pool, company, ano, opts = {}) {
  ...
  const [catsR, deparaR, grpR] = await Promise.all([
    ...
    pool.query('SELECT tipo, palavra_chave, categoria_nome FROM caixa_de_para WHERE empresa=$1', [company]),
  ...
  const [extR, cpR, pcR, vR, repR, fcR] = await Promise.all([
    pool.query('SELECT mes, dia, descricao, valor FROM caixa_extrato WHERE empresa=$1 AND ano=$2', [company, ano]),
    deparaCP.length > 0
      ? pool.query(
          somenteF
            ? 'SELECT fornecedor, saldo, data_vencimento FROM contas_pagar WHERE data_vencimento IS NOT NULL AND data_vencimento::date BETWEEN $1::date AND $2::date AND data_vencimento::date > $3::date'
            : 'SELECT fornecedor, saldo, data_vencimento FROM contas_pagar WHERE data_vencimento IS NOT NULL AND data_vencimento::date BETWEEN $1::date AND $2::date',
          somenteF ? [anoStart, anoEnd, today] : [anoStart, anoEnd])
  ...
  const months = [];
  for (let m = 1; m <= 12; m++) months.push({ mes: m, valores: {}, valores_previsao: {}, valores_previsao_pc: {} });
