...
Uso:
  python TINY_ESTOQUE_V3.py --empresa marcon --account tiny_marcon
  python TINY_ESTOQUE_V3.py --empresa lanzi  --account tiny_lanzi
"""
...
def refresh_token(cfg, account_name):
    ...
    print(f"[DEBUG] refresh={bool(refresh)}, client_id_len={len(client_id)}, client_secret_len={len(client_secret)}")
    ...
    if not r.ok:
        print(f"[ERRO] Refresh falhou: {r.status_code}")
        print(f"[ERRO] Response: {r.text[:500]}")
...
def main():
    ...
    parser.add_argument('--empresa', required=True, help='Empresa no db_config (ex: marcon, lanzi)')
    parser.add_argument('--account', required=True, help='Nome da conta Tiny (ex: tiny_marcon)')
    ...
    refresh = cfg.get(f"{account}_refresh")
    ...
    if needs_refresh:
        ...
        access_token, refresh, expires_in = refresh_token(cfg, account)
        save_token(engine, empresa, account, access_token, refresh, expires_in)
