...
DB_CONFIG = {
    ...
    "port"    : int(os.getenv("MARCON_PORT", "5432")),
...
def autenticar():
    ...
    # sessionid já está nos cookies da sessão
    return session
...
def extrair_estoque(session):
    ...
    while True:
        ...
        # Tenta descobrir formato da resposta
...
def salvar_estoque(engine, registros):
    ...
    # Mapeia campos (tenta adivinhar nomes comuns)
    linhas = []
    for r in registros:
...
def main():
    ...
    print("[INFO] Autenticando no Preço Certo...")
    session = autenticar()
