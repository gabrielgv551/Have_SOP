...
║  e sincroniza para a tabela caixa_extrato do Marcon          ║
╚══════════════════════════════════════════════════════════════╝

Vincule o cliente no app: extrator-bancario.vercel.app
  → Abra o cliente → clique em "Vincular ao Gestor..." → selecione "marcon"
...
# ─── Carrega .env do workspace root (pasta pai da Marcon/) ───────
...
# ─────────────────────────────────────────────────────────────────
# DESTINO: banco Marcon (caixa_extrato)
# ─────────────────────────────────────────────────────────────────
def obter_data_inicio_sync(conn_destino, item_id: str) -> str:
    if FULL_RELOAD:
        return FIRST_DATE
...
def garantir_belvo_link(conn_destino, item_id: str, institution: str):
    cur = conn_destino.cursor()
    cur.execute(
        """
...
def garantir_banco(conn_destino, nome: str) -> int:
    cur = conn_destino.cursor()
    cur.execute(
        """
...
def upsert_transacoes(conn_destino, transacoes: list, banco_id: int) -> int:
    if not transacoes:
        return 0
    rows = []
...
def atualizar_ultimo_sync(conn_destino, item_id: str):
    cur = conn_destino.cursor()
    cur.execute(
...
def main():
    ...
    with Spinner(f"Conectando a extratos e {EMPRESA.capitalize()}..."):
        ...
        conn_destino = psycopg2.connect(**DB_DESTINO,  **_ka)
    ...
    for client_id, client_name in clients:
        ...
        for item in items:
            ...
            print(f"     · {institution}")
            garantir_belvo_link(conn_destino, item_id, institution)
            banco_id  = garantir_banco(conn_destino, institution)
            date_from = obter_data_inicio_sync(conn_destino, item_id)
            ...
            if txs:
                with Spinner(f"Salvando {len(txs):,} transações..."):
                    n = upsert_transacoes(conn_destino, txs, banco_id)
                total_importado += n
                print(f"       ✔  {n:,} transações importadas")
            else:
                print(f"       ○  Nenhuma transação no período")

            atualizar_ultimo_sync(conn_destino, item_id)
