...
# ─────────────────────────────────────────────────────────────────
DB_CONFIG = {
    "host"    : os.getenv("SHOPGRA_HOST",     "37.60.236.200"),
    "port"    : os.getenv("SHOPGRA_PORT",     5432),
    "database": os.getenv("SHOPGRA_DB",       "shopgra"),
    "user"    : os.getenv("SHOPGRA_USER",     "postgres"),
    "password": os.getenv("SHOPGRA_PASSWORD", "131105Gv"),
}
...
def login(email: str, password: str) -> dict:
    ...
    plan_id     = data.get("customerPlanId") or (data.get("data") or {}).get("customerPlanId") or ""

    # Se planId não veio no JSON, extrai do payload JWT (campo customerPlanIds)
    if not plan_id and token:
        try:
            ...
            ids_raw = jwt_payload.get("customerPlanIds", "")
            if ids_raw:
                import re as _re
                nums = _re.findall(r'\d+', str(ids_raw))
                if nums:
                    plan_id = nums[0]
        except Exception:
            pass
    ...
    if not plan_id:
        plan_id = "507"  # fallback último recurso
