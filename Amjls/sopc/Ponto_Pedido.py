...
def ler_config(engine, modulo: str) -> dict:
    ...
    try:
        ...
        df = _pd.read_sql(
            f"SELECT chave, valor FROM sopc_config WHERE empresa='amjls' AND modulo='{modulo}'",
            engine
        )
...
def aplicar_leadtime_fornecedores(df: pd.DataFrame, engine) -> pd.DataFrame:
    ...
    try:
        sku_lt = pd.read_sql("""
            ...
            JOIN fornecedores_config fc ON fc.marca = s."Marca" AND fc.empresa = 'amjls'
...
def marcar_kits(df: pd.DataFrame, engine) -> pd.DataFrame:
    ...
    try:
        kits = carregar_kits(engine, empresa="amjls")
        if not kits:
            return df
