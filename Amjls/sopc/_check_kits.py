...
conn = psycopg2.connect(host="37.60.236.200", port=5432, dbname="Amjls", user="postgres", password="131105Gv")
cur = conn.cursor()

print("=== sku_kits (empresa=amjls) ===")
cur.execute("SELECT sku_kit, sku_componente, quantidade, ativo FROM sku_kits WHERE empresa='amjls'")
rows = cur.fetchall()
if rows:
    for r in rows:
