...
class Spinner:
    ...
    def __exit__(self, *_):
        ...
        self._thread.join()
        elapsed = time.time() - self._start
        sys.stdout.write(f"\r  [OK]  {self.msg}  ({elapsed:.1f}s)        \n")
        sys.stdout.flush()
...
def login(email: str, password: str) -> dict:
    ...
    # Decodificar o JWT para obter o customerPlanId dinamicamente
    plan_id = "507"
    if token:
        try:
            import base64
            ...
            if len(parts) == 3:
                ...
                if plan_ids:
                    plan_id = plan_ids[0]
                elif payload.get("customerPlanId"):
                    plan_id = str(payload.get("customerPlanId"))
        except Exception as e:
            print(f"  [AVISO] Não foi possível decodificar o token para obter planId: {e}")

    if not token:
        raise RuntimeError(f"Login OK mas sem token: {data}")

    print(f"  [OK]  Login bem-sucedido  (customerId={customer_id}, planId={plan_id})")
...
def baixar_todos(auth: dict, first_date: str, end_date: str) -> list:
    ...
    if total_pages == 1:
        print(f"  [OK] Apenas 1 página - {len(pedidos_p1):,} pedidos")
        return pedidos_p1
    ...
    with ThreadPoolExecutor(max_workers=WORKERS) as executor:
        ...
        for futuro in as_completed(futuros):
            ...
            bar = "#" * pct + "-" * (30 - pct)
            sys.stdout.write(f"\r  [{bar}] {concluidas}/{total_pages} paginas  |  {total_acumulado:,} registros   ")
            sys.stdout.flush()
    ...
    print()  
    print(f"  [OK] Download concluído: {len(todos_pedidos):,} pedidos")
...
def processar(pedidos: list) -> pd.DataFrame:
    ...
    for pedido in pedidos:
        ...
        for produto in produtos:
            ...
            linha.update({
                ...
                "Total Venda"        : produto.get("valueWithoutDiscount", 0),
    ...
    if df.empty:
        print("  ⚠  Nenhum pedido processado. Dataframe vazio.")
        # Retorna dataframe vazio com as colunas esperadas para não quebrar outros pontos
        return pd.DataFrame(columns=[
            "Order ID", "Produto ID", "Sku", "Sku Anterior", "Nome Produto", 
            "Categoria", "NCM", "Canal de venda", "Canal Apelido", "Data", 
            "Status", "Numero Ecommerce", "Cliente", "Estado", "Cidade", 
            "Metodo Envio", "App Integracao", "Quantidade Vendida", "Quantidade Pedido", 
            "Estoque Total", "Custo Total", "Total Venda", "Valor Desconto", 
            "Margem Produto", "Valor Liquido Prod", "Perc Custo", "Perc Margem", 
            "Imposto Produto", "Frete Pago Prod", "Comissao Produto", "Frete Recebido", 
            "Frete Pago", "Comissao Pedido", "Diferenca Frete", "Taxas", 
            "Embalagem", "Repasse Financeiro", "Margem Contribuicao", "Valor Liquido", 
            "Total Custo Pedido", "Total Venda Pedido", "Ano", "Mes", 
...
def obter_data_ultimo_sync(engine) -> str:
    try:
        ...
        if resultado:
            ...
            print(f"  [OK]  Último registro no banco: {pd.to_datetime(resultado).strftime('%d/%m/%Y')}")
...
def upsert_banco(engine, df: pd.DataFrame):
    if df.empty:
        print("  ○  Nenhum registro para salvar no banco.")
        return
    ...
    print(f"  [OK]  Upsert concluído  |  sync registrado")
...
def obter_credenciais() -> tuple[str, str]:
    ...
    try:
        ...
        if email and password:
            print("  [OK] Credenciais carregadas da tabela configuracoes.")
            return email, password
    except Exception as e:
        print(f"[AVISO] {e}")
...
def main():
    ...
    _sep("=")
    print(f"  [OK] ETL finalizado em {duracao}s")
