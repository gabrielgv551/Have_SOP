...
PC_EMAIL     = os.getenv("PRECOCERTO_EMAIL",    "gabriel.viana@have.com.br")
...
def autenticar_playwright() -> dict:
    ...
    with sync_playwright() as p:
        ...
        _scr2 = os.path.join(os.path.dirname(__file__), "pos_login.png")
        page.screenshot(path=_scr2)
        print(f"  [debug] Screenshot pós-login: {_scr2}")

        # Seleção automática de empresa (conta consultora HAVE)
        EMPRESA_NOME = os.getenv("PRECOCERTO_EMPRESA", "AUTOMOZ COMERCIAL DISTRIBUIDORA LTDA")
        try:
            # Aguarda página "Escolher empresa"
            page.wait_for_selector("text=Escolher empresa", timeout=6_000)
            print(f"  Página 'Escolher empresa' detectada — selecionando: {EMPRESA_NOME}")

            # Abre o dropdown select2
            page.locator(".select2-selection").first.click()
            page.wait_for_timeout(600)

            # Clica na opção correta na lista do select2
            page.locator(".select2-results__option", has_text=EMPRESA_NOME).first.click()
            page.wait_for_timeout(3000)  # aguarda redirect após selecionar

            _scr3 = os.path.join(os.path.dirname(__file__), "pos_empresa.png")
            page.screenshot(path=_scr3)
            print(f"  ✔  Empresa selecionada | URL: {page.url}")
        except Exception as _e:
            print(f"  [info] Seleção de empresa: {_e}")
...
def baixar_export_xlsx(session: requests.Session, date_after: str, date_before: str) -> pd.DataFrame:
    ...
    for tentativa in range(3):
        ...
        if r.status_code in (401, 403):
            if tentativa < 2:
                print(f"    [sessão {r.status_code}] Re-autenticando (Playwright)...")
                auth_dict = autenticar_playwright()
                for name, value in auth_dict["cookies"].items():
                    session.cookies.set(name, value, domain="sys.precocerto.co")
                csrf = session.cookies.get("csrftoken", "")
                h["x-csrftoken"] = csrf
                continue

        if r.status_code in (400, 502, 504) or r.status_code >= 400:
            if r.status_code not in (400, 502, 504) and tentativa < 2:
                continue # Tenta de novo se for outro erro
            # 400, 504 ou esgotou tentativas -> dividir em quinzenas e combinar
            subs = _split_periodo(date_after, date_before)
            
            # Evitar loop infinito: se já é <= 15 dias, corta ao meio
            if len(subs) == 1 and subs[0][0][:10] == date_after[:10] and subs[0][1][:10] == date_before[:10]:
                d0 = datetime.strptime(date_after, "%Y-%m-%d")
                d1 = datetime.strptime(date_before, "%Y-%m-%d")
                delta_days = (d1 - d0).days
                if delta_days == 0:
                    raise RuntimeError(f"Erro {r.status_code} em único dia: {date_after}")
                mid_date = d0 + timedelta(days=delta_days // 2)
                subs = [
                    (d0.strftime("%Y-%m-%d"), mid_date.strftime("%Y-%m-%d")),
                    ((mid_date + timedelta(days=1)).strftime("%Y-%m-%d"), d1.strftime("%Y-%m-%d"))
                ]
            
            print(f"    [erro {r.status_code}] Dividindo em {len(subs)} janelas menores...")
            dfs = []
            ...
            print(f"    ✔  {len(combined):,} linhas combinadas de {len(subs)} janelas")
            return combined
...
def main():
    ...
    if REPROCESS:
        ...
        auth_dict = autenticar_playwright()
        # Transfere cookies do Playwright para uma requests.Session
        ...
        print(f"  ✔  Session pronta com {len(auth_dict['cookies'])} cookies do Playwright")
