...
def calcular_padrao_semanal(df: pd.DataFrame):
    ...
    agg = df.groupby(["unique_id", "mes", "semana"])["quantidade"].sum().reset_index()
    total_mes = agg.groupby(["unique_id", "mes"])["quantidade"].sum().rename("total_mes").reset_index()
    agg = agg.merge(total_mes, on=["unique_id", "mes"])
    agg["pct"] = agg["quantidade"] / agg["total_mes"].replace(0, np.nan)

    media_pct = agg[agg["total_mes"] > 0].groupby(["unique_id", "semana"])["pct"].mean().unstack(fill_value=0)
    for s in [1, 2, 3, 4]:
    ...
    n_meses = agg[agg["total_mes"] > 0].groupby("unique_id")["mes"].nunique().rename("n_meses").reset_index()
...
def calcular_padrao_dow(df: pd.DataFrame):
    ...
    agg = df.groupby(["unique_id", "dow"])["quantidade"].sum().unstack(fill_value=0)
    for d in range(7):
    ...
    total_qtd = df.groupby("unique_id")["quantidade"].sum().rename("total_qtd").reset_index()
...
# ─────────────────────────────────────────────────────────────────
def gerar_forecast_diario(df_forecast, df_sem, fallback_sem, df_dow, fallback_dow) -> pd.DataFrame:
    ...
    FAIXAS_SEM = {1: (1, 7), 2: (8, 14), 3: (15, 21), 4: (22, 31)}
    ...
    for _, row in df_forecast.iterrows():
        ...
        qty_int = int(round(qty_mes))

        todos_dias   = list(range(1, n_dias + 1))
        pesos_totais = []
        sem_por_dia  = []
        for dia in todos_dias:
            sem_num = next(s for s, (di, df2) in FAIXAS_SEM.items() if di <= dia <= df2)
        ...
        pesos_totais = pesos_totais / soma if soma > 0 else np.ones(n_dias) / n_dias
...
def main():
    print("=" * 60)
    print("  S&OP Intelligence · Forecast Diário  ·  Autoequip")
