...
║  Banco     : PostgreSQL local (Supershop)                        ║
...
# CONFIGURAÇÃO
# ─────────────────────────────────────────────
DB_CONFIG = get_db_config('supershop')
...
def ler_config(engine, modulo: str) -> dict:
    ...
    try:
        ...
        df = _pd.read_sql(
            f"SELECT chave, valor FROM sopc_config WHERE empresa='Supershop' AND modulo='{modulo}'",
            engine
        )
...
def aplicar_leadtime_fornecedores(df: pd.DataFrame, engine) -> pd.DataFrame:
    ...
    try:
        sku_lt = pd.read_sql("""
            ...
            JOIN fornecedores_config fc ON fc.marca = s."Marca" AND fc.empresa = 'Supershop'
